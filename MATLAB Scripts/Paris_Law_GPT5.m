%% Pure ReAct Agent for Paris Law Fitting - PORTABLE VERSION
...
ai_model = openAIChat(['You are an agent helping fit Paris law to fatigue crack growth data. ' ...
    ...
    'Be concise and specific.'], ModelName="gpt-5");
...
fprintf('\n=== Pure ReAct Agent with Structured Tools Started (GPT-5 Optimized) ===\n');
...
fprintf('*** GPT-5 CONFIGURATION: Temperature=0 for deterministic output ***\n\n');
...
function prompt = build_react_prompt(agent_state, tools)
    ...
    % Create a prompt that leverages GPT-5's enhanced reasoning capabilities
    prompt = [ ...
        'You are a ReAct agent for fatigue crack growth data fitting. ' ...
        'You have expert knowledge of fracture mechanics and the Paris law.\n\n' ...
        'TASK: ' agent_state.context '\n\n' ...
        'DATA FILE LOCATION: ' agent_state.filename '\n' ...
        'IMPORTANT: Use this EXACT filename when calling load_data.\n\n' ...
        'AVAILABLE TOOLS:\n' tool_descriptions ...
        '\nTOOL USAGE NOTES:\n' ...
        '- Tools that require data (fit_model, validate_fit, create_plots) automatically use\n' ...
        '  the currently loaded data and selected region from the agent state.\n' ...
        '- The auto_select_region tool uses slope analysis to identify Region II without any input.\n' ...
        '- The generate_function tool MUST produce the Paris law equation from your knowledge.\n' ...
        '- No default or fallback equations are provided - you must generate the correct form.\n\n' ...
        'CURRENT STATE:\n'];
    ...
    if agent_state.function.ready
        ...
        prompt = [prompt '  *** MUST generate equation from your knowledge - no default provided ***\n'];
    end
    ...
    % Recent history with reasoning context
    ...
    % Final reasoning instruction - optimized for GPT-5
    prompt = [prompt ...
        '\nBased on the current state and history, determine the next logical action.\n' ...
        'Consider the Paris law fundamentals:\n' ...
        '- Region II follows da/dN = C(ΔK)^m in log-log space\n' ...
        '- Typical m values range from 2-4 for metals\n' ...
        '- The function must have ΔK as the first parameter\n\n' ...
...
function [result, agent_state] = action_auto_select_region(~, agent_state)
    ...
    try
        ...
        % Paris law slope range (based on fracture mechanics principles)
        min_acceptable_slope = 2.0;
        max_acceptable_slope = 4.0;
        
        % Find regions with slopes in acceptable range AND good linearity
        min_r2 = 0.95;
        valid_region = (slopes >= min_acceptable_slope) & (slopes <= max_acceptable_slope) & (r2_values >= min_r2);
        
        % If no region meets both criteria, relax R² requirement
        if ~any(valid_region)
            fprintf('No region with R² ≥ 0.95 found, relaxing R² threshold to 0.90...\n');
            min_r2 = 0.90;
            valid_region = (slopes >= min_acceptable_slope) & (slopes <= max_acceptable_slope) & (r2_values >= min_r2);
        end
        
        % If still no region, use slope stability as primary criterion
        if ~any(valid_region)
            fprintf('Using slope stability as primary criterion...\n');
...
function [result, agent_state] = action_generate_paris_function(~, agent_state)
    ...
    try
        ...
        equation_model = openAIChat( ...
            'You are a materials science expert with deep knowledge of fracture mechanics and the Paris law.', ...
            ModelName="gpt-5");
        ...
        %% STEP 1: Ask LLM for the equation form - Leverage GPT-5's enhanced knowledge
        equation_prompt = [ ...
            'As a fracture mechanics expert, what is the exact mathematical form of the Paris law equation?\n\n' ...
            'The Paris law describes Region II fatigue crack growth, relating da/dN (crack growth rate) to ΔK (stress intensity factor range).\n\n' ...
            'Respond with EXACTLY:\n' ...
            '1. The equation in the form: da/dN = [expression]\n' ...
            '2. Parameter 1: name and physical meaning\n' ...
            '3. Parameter 2: name and physical meaning\n' ...
            '4. Units for each variable\n\n' ...
        ...
        code_model = openAIChat( ...
            'You are a MATLAB code generator. You output ONLY valid MATLAB code with no explanations.', ...
            ModelName="gpt-5");
        
        %% STEP 3: Ask LLM to convert to MATLAB function
        code_prompt = [ ...
            'Based on this Paris law equation:\n\n' ...
            char(equation_form) '\n\n' ...
            ...
            '2. FIRST parameter: independent variable ΔK\n' ...
            '3. REMAINING parameters: material constants in the order they appear in your equation\n' ...
            '4. Use element-wise operations: .*, ./, .^\n' ...
            '5. Output must be a single line of MATLAB code\n\n' ...
            'OUTPUT FORMAT:\n' ...
            'Output EXACTLY ONE LINE of MATLAB code.\n' ...
            'NO additional text, comments, markdown, or explanations.'];
        ...
        try
            ...
            if isempty(param_names) || length(param_names) < 2
                error('Function must have at least 2 parameters (independent variable + material constants)');
            end
...
%% Robust response cleaning function
...
function [result, agent_state] = action_create_paris_plots(~, agent_state)
    ...
    try
        ...
        SIF_fit = logspace(log10(DeltaK_min), log10(DeltaK_max), 200)';
        
        try
            if n_params == 2
                dadN_fit = crack_growth_model(SIF_fit, C_hat, m_hat);
            else
                dadN_fit = crack_growth_model(SIF_fit, p_fit(:));
            end
        catch
            dadN_fit = crack_growth_model(SIF_fit, p_fit(:));
        end
        ...
        loglog(SIF_fit, dadN_fit, 'r-', 'LineWidth', 2.5, 'DisplayName', 'Paris Law Fit');
...
function [result, agent_state] = action_export_paris_results(input, agent_state)
    ...
    try
        ...
        if strcmp(format, 'text')
            ...
            fprintf(fid, 'Paris Law Fitting Results\n');
            fprintf(fid, '=========================\n\n');
            fprintf(fid, 'Date: %s\n\n', datestr(now));
            fprintf(fid, 'Region Selection: %s\n', agent_state.region.selection_method);
            fprintf(fid, 'Detected Slope Range: [%.2f, %.2f]\n\n', ...
                agent_state.region.detected_slope_range(1), ...
                agent_state.region.detected_slope_range(2));
            fprintf(fid, 'Generated Function: %s\n', agent_state.function.func_str);
            fprintf(fid, 'Parameter Order: %s\n\n', strjoin(agent_state.function.param_names, ', '));
            fprintf(fid, 'Fitted Parameters:\n');
            fprintf(fid, '  C = %.6e m/cycle\n', results.C);
            fprintf(fid, '  m = %.4f\n\n', results.m);
            fprintf(fid, 'Goodness of Fit:\n');
            fprintf(fid, '  R² (linear) = %.6f\n', results.rsquared);
            fprintf(fid, '  R² (log)    = %.6f\n', results.rsquared_log);
            fprintf(fid, '  Adjusted R² = %.6f\n', results.rsquared_adj);
            fprintf(fid, '  RMSE        = %.6e m/cycle\n', results.rmse);
            fclose(fid);
        end
