%% Agent for Kuhn Equation Fitting (Just LLM Knowledge)
% This version uses the ReAct framework but asks the LLM directly for Kuhn's equation
...
fprintf('=== Pure ReAct Agent for Kuhn Equation Fitting Started ===\n');
...
%% Get Kuhn equation from LLM knowledge
function [result, agent_state] = action_get_kuhn_equation(~, agent_state)
    result = struct();
    
    try
        % Create model for getting equation
...
%% Convert LaTeX to MATLAB function
function [result, agent_state] = action_convert_to_matlab(~, agent_state)
    ...
    try
        ...
        % Build prompt using simple concatenation (no sprintf to avoid formatting issues)
        prompt = [ ...
...
function [result, agent_state] = action_create_plots(~, agent_state)
    ...
    try
        ...
        title('Kuhn Model Fit from LLM Knowledge');
