%% Helicene Strain-Modified Kuhn Equation with AI Agent
% This script implements a ReAct AI agent that modifies the Kuhn equation
% to model strain effects on helicene HOMO-LUMO gaps, using DFT data for fitting.
% Note: Guo dataset is intentionally commented out as requested.

clc; clear all; close all;

%% Get the script's directory and set up file paths
scriptDir = fileparts(which(mfilename('fullpath')));
% guoFile = fullfile(scriptDir, 'Helicene_Guo.csv'); % COMMENTED OUT - Guo dataset not used
...
fprintf('Note: Guo dataset is not used in this version (commented out)\n');

%% Initialize AI Model for ReAct Reasoning - GPT-5 Version (no temperature)
ai_model = openAIChat([ ...
...
% Tool 1: Load Experimental Data (DFT only)
tools.load_data = struct();
tools.load_data.description = 'Load strain-gap experimental data from DFT CSV file only';
...
tools.plot_results.description = 'Plot DFT data vs model predictions';
...
% File paths - DFT only
% agent_state.guo_file = guoFile; % COMMENTED OUT
...
% Data state - Guo data fields are empty
agent_state.data = struct('loaded', false, 'strain_dft', [], 'gap_dft', []);
...
fprintf('=== ReAct Agent for Strain-Modified Kuhn Equation (GPT-5) Started ===\n');
fprintf('BASE EQUATION: HARDCODED (provided to LLM)\n');
fprintf('Task: Generate strain modifications to match DFT data\n');
fprintf('Data file: %s\n', agent_state.dft_file);
fprintf('Note: Guo dataset is NOT used (commented out)\n');
...
function prompt = build_react_prompt(agent_state)
    ...
    prompt = [ ...
        ...
        'NOTE: The Guo experimental dataset is NOT used in this version (commented out).\n\n' ...
        'AVAILABLE TOOLS:\n' ...
        '- load_data: Load DFT data (input: {}) - automatically finds file in script folder\n' ...
    ...
    if agent_state.data.loaded
        prompt = [prompt sprintf('- DFT data loaded: %d points\n', ...
            length(agent_state.data.strain_dft))];
    else
    ...
    prompt = [prompt ...
        ...
        'The DFT data file is in the same folder as this script. Use load_data with empty input to load it automatically.\n' ...
        'The Guo dataset is NOT available in this version.\n' ...
    ...
    if result.success
        switch tool_name
            case 'load_data'
                ...
                agent_state.data.strain_dft = result.metadata.strain_dft;
                agent_state.data.gap_dft = result.metadata.gap_dft;
...
function [result, agent_state] = action_load_data(input, agent_state)
    ...
    try
        % Use the file paths stored in agent_state
        ...
        % Load DFT data only (Guo data is commented out)
        ...
        agent_state.data.loaded = true;
        agent_state.data.strain_dft = strain_dft;
        agent_state.data.gap_dft = gap_dft;
        
        result.message = sprintf('Loaded DFT data: %d points (strain %.1f-%.1f%%)', ...
            length(strain_dft), min(strain_dft), max(strain_dft));
        
        result.success = true;
        result.metadata = struct('strain_dft', strain_dft, 'gap_dft', gap_dft);
...
function [result, agent_state] = action_finalize(~, agent_state)
    ...
    if agent_state.function.ready && agent_state.data.loaded && ~isempty(agent_state.results.gap_avg)
        ...
        result.message = sprintf([ ...
            ...
            'Test passed: %d\n' ...
            'Plot created with DFT data comparison'], ...
...
function create_final_plot(agent_state)
    ...
    % Plot DFT data only (Guo data is commented out)
    plot(agent_state.data.strain_dft, agent_state.data.gap_dft, 'bs-', ...
    ...
    title('Helicene Strain-Dependent HOMO-LUMO Gap: DFT vs AI-Modified Kuhn Equation');
