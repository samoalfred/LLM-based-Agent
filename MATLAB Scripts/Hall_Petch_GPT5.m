%% Agent for Hall-Petch Fitting - PORTABLE VERSION
...
% Tool 2: Generate Function (MUST come from LLM - NO FALLBACK)
tools.generate_function = struct();
tools.generate_function.description = 'Generate Hall-Petch function using LLM (MUST come from LLM - no fallback)';
...
%% Function generation with proper context but NO hardcoded equation
...
function clean = clean_matlab_response(raw)
    ...
    % Last resort - if we see a line with d^(-0.5) or similar
    for i = 1:length(lines)
        line = strtrim(lines{i});
